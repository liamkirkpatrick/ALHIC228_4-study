...
adj = pd.DataFrame()
adj['ID'] = df['ID']
adj['topdepth'] = (df['rawdepth_top_m'] 
                   + ave_intopage/1000 * np.sin(side_angle*np.pi/180)
                   + ave_crosspage/1000 * np.sin(top_angle*np.pi/180))
adj['middepth'] = ((df['rawdepth_top_m']+df['rawdepth_bottom_m'])/2
                   + ave_intopage/1000 * np.sin(side_angle*np.pi/180)
                   + ave_crosspage/1000 * np.sin(top_angle*np.pi/180))
adj['botdepth'] = (df['rawdepth_bottom_m'] 
                   + ave_intopage/1000 * np.sin(side_angle*np.pi/180)
                   + ave_crosspage/1000 * np.sin(top_angle*np.pi/180))

#%% Save adjusted depths

adj.to_csv(path_to_data+'dip_adjusted_depths.csv')